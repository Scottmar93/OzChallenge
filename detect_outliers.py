...
screens = [
    l.split()[-3:]
    for l in subprocess.check_output(["xrandr"]).decode("utf-8").strip().splitlines()
    if " connected" in l
]
for s in screens:
...
if preprocess_data == True:
    ...
    if mpiRank == 0:
        print(
            "Original mesh sizes (x, y): (%f, %f), new mesh sizes (x, y): (%f, %f)"
            % (
                uBx[1] - uBx[0],
                uBy[1] - uBy[0],
                (xymax[0] - xymin[0]) / Nx,
    ...
    spB1V = RectBivariateSpline(
        uB1Vx, uB1Vy, B1V[:, 2].reshape((len(uB1Vy), len(uB1Vx))).T
    )
    ...
    if mpiRank == 0:
        ...
        np.savez(
            "field_data.npz", B=B_reduced, B1V=B1V_reduced, g=g_reduced, mines=mines
        )
...
if load_results == False:
    ...
    for iteration, (i, j) in enumerate(my_Np):
        ...
        T = np.vstack(
            [
                BB[aux1, :][:, aux2].flatten(),
                gg[aux1, :][:, aux2].flatten(),
                gradBB[aux1, :][:, aux2].flatten(),
                gradgg[aux1, :][:, aux2].flatten(),
                BB1V[aux1, :][:, aux2].flatten(),
            ]
        ).T
        clf.fit(T)
        ...
        s = int(k / 4)
        outliers[aux1[s] : aux1[-s], aux2[s] : aux2[-s]] |= y_pred[
            s:-s, s:-s
        ]  # logical OR
        outliers_score[aux1[s] : aux1[-s], aux2[s] : aux2[-s]] = np.minimum(
            outliers_score[aux1[s] : aux1[-s], aux2[s] : aux2[-s]],
            y_pred_score[s:-s, s:-s],
        )

        if mpiRank == 0:
            stdout.write("\r")
            stdout.write(
                "Outlier detection. Progress: [%-50s] %d%%"
                % (
                    "=" * int(round(50 * (iteration + 1) / len(my_Np))),
                    int(round(100 * (iteration + 1) / len(my_Np))),
                )
            )
            stdout.flush()
    ...
    np.savez(
        "outlier_data_temp%d.npz" % mpiRank,
        outliers=outliers,
    ...
    if mpiRank == 0:
        ...
        outliers_scores = reduce(
            np.minimum, [item["outliers_score"] for item in outlier_datas]
        )
